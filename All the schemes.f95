program Allschemes
    ...
    integer,parameter::n=10!! Number of grids 
    real(8),dimension(n)::b,x,d,a,c,w
    real(8)::aE,aW,Dif,F,bL,bR,gamma,Ax,Velocity,Density,s,sp1,sp2,zero,Peclet,Length
    zero=0
    s=n
    Length=1 !!Length of the rod
    gamma=0.1 !!Difussion coefficient
    Velocity=1 !!Velocity of the fluid in m/s
    ...
    write (*,*) 'Select differencing scheme you would like to use' 
    write (*,*) 'Press 1 - Central Differencing scheme'
    write (*,*) 'Press 2 - Upwind Differencing scheme'
    write (*,*) 'Press 3 - Power Law Differencing scheme'
    read (*,*) scheme
    select case (scheme)
        case(1) !Central Differencing Scheme
            aE=Dif-(F/2) 
            aW=Dif+(F/2)
            sp1=(2*Dif)+F
            sp2=(2*Dif)-F
            
        case(2) !Upwind Differencing scheme
            if (F>=0) then
                aE=Dif 
                aW=Dif+F
            else if (F<=0) then
        ...
        case(3) !Power low scheme
            aW=Dif*MAX(zero,(1-(0.1*(Peclet)))**5)+MAX(F,zero)
            aE=Dif*MAX(zero,(1-(0.1*(Peclet)))**5)+MAX(-F,zero)
            sp1=(2*Dif)+F
            sp2=(2*Dif)
